...
if(NOT pp_rc EQUAL 0)
    ...
    foreach(_e IN LISTS _errs)
        # La ruta puede venir absoluta -- y en Windows, empezando por "C:" --
        # asi que no vale partir por el primer ':'.  Se ancla el final: numero
        # de linea, numero de columna y la palabra error.
        if(_e MATCHES "^[ \t]*(.+):([0-9]+):[0-9]+: error")
            set(_f "${CMAKE_MATCH_1}")
            set(_n "${CMAKE_MATCH_2}")

            # Si la ruta ya es utilizable tal cual, se usa; si es relativa, se
            # busca dentro de las rutas de inclusion.  Sin la primera rama, un
            # error en una cabecera del sistema -- que es justo el caso que esta
            # prueba existe para cazar -- se quedaba sin contexto.
            set(_ruta "")
            if(EXISTS "${_f}")
                set(_ruta "${_f}")
            else()
            ...
            if(_ruta)
                ...
                math(EXPR _desde "${_n} - 12")
                math(EXPR _hasta "${_n} + 2")
                ...
                message(STATUS "--- ${_f}:${_n} (no se pudo leer el fichero) ---")
            endif()
        endif()
    endforeach()
    message(FATAL_ERROR
